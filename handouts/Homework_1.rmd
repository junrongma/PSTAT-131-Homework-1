...
m_v_by_chemical <- algae %>%
  ...
  gather(factor_key = TRUE) %>% group_by(key) %>% na.omit() %>%
...
# algae.del <- algae %>% na.omit() - Easier and also works

nrow(algae.del)
```

        #. There are $`r nrow(alge.del)`$ observations in `algae.del`
...
```{r 3c) Imputing values, indent = indent2}
algae.med <- algae %>%
