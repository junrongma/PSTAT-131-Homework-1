...
```{r 1.a) Number of observations, indent = indent2}
...
```{r 1.b) Missing Values, indent = indent2}
algae_missing_values <- algae %>%
  # Filters on all columns and predicate returns true if any column contains an NA
  filter_all(any_vars(is.na(.)))

algae_missing_values
```

```{r 1.b) Mean and Variance of each chemical, indent = indent2}
m_v_by_chemical <- algae %>%
  # Selects All chemicals
  select(mnO2:Chla) %>%
  # Groups by key THEN removes NA's
  gather() %>% group_by(key) %>% na.omit() %>%
